...
# def generate_luts():
#     hsv_lut = np.zeros((256, 1, 3), dtype=np.uint8)
#     rainbow_lut = np.zeros((256, 1, 3), dtype=np.uint8)

#     for i in range(256):
#         hsv_lut[i, 0] = (i, 255, 255)  # HSV color space
#         rainbow_lut[i, 0] = (i, 255, 255)  # HSV representation for rainbow

#     hsv_lut = cv2.cvtColor(hsv_lut, cv2.COLOR_HSV2BGR)
#     rainbow_lut = cv2.cvtColor(rainbow_lut, cv2.COLOR_HSV2BGR)

#     inv_hsv_lut = create_inverse_lut(hsv_lut)
#     inv_rainbow_lut = create_inverse_lut(rainbow_lut)

#     return hsv_lut, rainbow_lut, inv_hsv_lut, inv_rainbow_lut
